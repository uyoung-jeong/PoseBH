...
class TopDownMoE(BasePose):
    ...
    def init_weights(self, pretrained=None, multihead_pretrained=None):
        ...
        res = self.backbone.init_weights(pretrained)
        ...
        if res is False:
            if isinstance(pretrained, str):
                ...
                print("Initializing entire model with pretrained weights")
    ...
    def forward_train(self, img, target, target_weight, img_metas, **kwargs):
        ...
        for idx in range(1, self.keypoint_heads_cnt):
            ...
            #print(f"head idx: {idx}, output_select: {output_select.shape}, target_select: {target_select.shape}, target_weight_select: {target_weight_select.shape}")
            out_n_joints = output_select.size(1)
            tgt_n_joints = target_select.size(1)
            if out_n_joints < tgt_n_joints:
