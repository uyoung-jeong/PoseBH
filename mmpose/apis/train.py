...
def train_model(model,
    ...
    # wrap model with fp16 if fp16 config is not None
    """
    ...
    if distributed:
        ...
        # if freeze multihead weight, set find_unused_parameters True
        custom_hook_keys = [e['type'] for e in custom_hooks]
        if 'MultiheadFreezeHook' in custom_hook_keys:
            find_unused_parameters = True
        elif 'BackboneFreezeHook' in custom_hook_keys:
            find_unused_parameters = True
    ...
    for custom_hook in custom_hooks:
        runner.register_hook_from_cfg(custom_hook)
