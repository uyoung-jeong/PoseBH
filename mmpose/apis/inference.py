...
def _inference_single_pose_model(model,
    ...
    for bi, bbox in enumerate(bboxes):
        if centers is not None:
            center, scale = centers[bi], scales[bi]
        else:
    ...
    if 'cluster_heatmap' in result.keys():
        return result['preds'], result['output_heatmap'], result['proto_heatmap'], result['cluster_heatmap']
    elif 'proto_heatmap' in result.keys():
        return result['preds'], result['output_heatmap'], result['proto_heatmap']
...
def inference_top_down_pose_model(model,
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        ...
        res = _inference_single_pose_model(
            model,
            img_or_path,
            bboxes_xywh,
            dataset=dataset,
        ...
        if len_res == 2:
            poses, heatmap = res
        elif len_res == 3: # proto
            poses, heatmap, proto_heatmap = res
        elif len_res == 4: # proto + cluster prototype
            poses, heatmap, proto_heatmap, cluster_heatmap = res

        if return_heatmap:
            h.layer_outputs['heatmap'] = heatmap
            if len_res == 3:
                h.layer_outputs['proto_heatmap'] = proto_heatmap
            elif len_res == 4:
                h.layer_outputs['proto_heatmap'] = proto_heatmap
                h.layer_outputs['cluster_heatmap'] = cluster_heatmap
...
def vis_pose_result(model,
    ...
    if dataset_info is not None:
        ...
        if dataset in ('TopDownCocoDataset', 'BottomUpCocoDataset',
        ...
        elif dataset == 'TopDownCrowdPoseDataset':
            skeleton = [[10, 8], [8, 6], [11, 9], [9, 7], [6, 7], [0, 6], [1, 7], [0, 1], [0, 2], [1, 3], [2, 4], [3, 5], [12, 13], [1, 13], [0, 13]]
            pose_link_color = palette[[16, 16, 0, 0, 9, 9, 9, 9, 16, 0, 16, 0, 9, 9, 9]]
            pose_kpt_color = palette[[9, 9, 9, 9, 9, 16, 0, 16, 0, 16, 0, 16, 0, 16]]
        elif dataset in ('OneHand10KDataset', 'FreiHandDataset',
                         'PanopticDataset'):
            ...
            pose_link_color = palette[[
                0, 0, 16, 16, 0, 16, 4, 4, 4, 4, 8, 0, 16, 0, 16, 0, 16, 0, 16, 0, 16, 4, 4
            ]]
            pose_kpt_color = palette[[
                4, 0, 16, 4, 0, 16, 4, 0, 16, 4, 0, 16, 8, 0, 16, 8, 0, 16, 0, 16, 0, 16, 0, 16
            ]]
        elif dataset == 'TopDownPoseTrack18Dataset':
        ...
        elif dataset in 'AnimalAP10KDataset':
            skeleton = [
                    [0, 1], [0, 2], [1, 2], [2, 3], [3, 4], [3, 5], [5, 6], [6, 7], [3, 8], 
                    [8, 9], [9, 10], [4, 11], [11, 12], [12, 13], [4, 14], [14, 15], [15, 16]]

            pose_link_color = palette[[0] * 17]
            pose_kpt_color = palette[[0] * 17]
        else:
            NotImplementedError()
    ...
    #if hasattr(model, 'show_result_pyplot'):
    if '.pdf' in out_file and hasattr(model, 'show_result_pyplot'):
        img = model.show_result_pyplot(
            img,
            result,
            skeleton,
            radius=radius,
