...
@deprecated_api_warning({'pose_limb_color': 'pose_link_color'})
def imshow_keypoints_pyplot(img,
                     pose_result,
                     skeleton=None,
                     kpt_score_thr=0.3,
                     pose_kpt_color=None,
    ...
    Args:
            img (str or Tensor): The matplotlib image plot.
            pose_result (list[kpts]): The poses to draw. Each element kpts is
                a set of K keypoints as an Kx3 numpy.ndarray, where each
                keypoint is represented as x, y, score.
            kpt_score_thr (float, optional): Minimum score of keypoints
        ...
        if skeleton is not None and pose_link_color is not None:
            ...
            for sk_id, sk in enumerate(skeleton):
                ...
                if (pos1[0] > 0 and pos1[0] < img_w and pos1[1] > 0
                        ...
                        and kpts[sk[1], 2] > kpt_score_thr):
                    color = tuple(int(c) for c in pose_link_color[sk_id])
                    if show_keypoint_weight: # this should not be executed
                        ...
                        color = [e/255.0 for e in color]
                        color.reverse()
                        img.axes.plot((pos1[0], pos2[0]), (pos1[1], pos2[1]), color=color, linewidth=thickness)
            ...
            for kid, kpt in enumerate(kpts):
                ...
                if kpt_score > kpt_score_thr:
                    ...
                    if show_keypoint_weight:
                        ...
                        color = [e/255.0 for e in color]
                        color.reverse()
                        img.axes.plot(x_coord, y_coord, marker='o', color=color, markersize=radius)
