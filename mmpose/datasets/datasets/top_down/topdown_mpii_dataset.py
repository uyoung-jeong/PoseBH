...
class TopDownMpiiDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def evaluate(self, results, res_folder=None, metric='PCKh', **kwargs):
        ...
        if '_fold' in self.ann_file:
            fold_i = self.ann_file[-6]
            gt_file = osp.join(osp.dirname(self.ann_file), f'mpii_gt_val_fold{fold_i}.mat')
        elif '_multiocc' in self.ann_file:
            gt_file = osp.join(osp.dirname(self.ann_file), f'mpii_gt_val_multiocc.mat')
